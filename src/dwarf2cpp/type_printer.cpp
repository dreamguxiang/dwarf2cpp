...
void DWARFTypePrinter::appendTypeTagName(dwarf::Tag T)
{
    ...
    OS << TagStr.substr(Prefix.size(), TagStr.size() - (Prefix.size() + Suffix.size())) << " ";
}
...
void DWARFTypePrinter::appendArrayType(const DWARFDie &D)
{
    for (const DWARFDie &C : D.children()) {
        ...
        if (std::optional<DWARFFormValue> LV = D.getDwarfUnit()->getUnitDIE().find(DW_AT_language)) {
            if (std::optional<uint64_t> LC = LV->getAsUnsignedConstant()) {
                if ((DefaultLB = LanguageLowerBound(static_cast<dwarf::SourceLanguage>(*LC)))) {
                    if (LB && *LB == *DefaultLB) {
                        LB = std::nullopt;
                    }
                }
...
{
    while (D && (D.getTag() == DW_TAG_const_type || D.getTag() == DW_TAG_volatile_type)) {
...
void DWARFTypePrinter::appendPointerLikeTypeBefore(DWARFDie D, DWARFDie Inner, StringRef Ptr)
{
...
DWARFDie DWARFTypePrinter::appendUnqualifiedNameBefore(DWARFDie D, std::string *OriginalFullName)
{
    Word = true;
    if (!D) {
        OS << "void";
    ...
    DWARFDie InnerDIE;
    auto Inner = [&] {
        return InnerDIE = resolveReferencedType(D);
    };
...
void DWARFTypePrinter::appendUnqualifiedNameAfter(DWARFDie D, DWARFDie Inner, bool SkipFirstParamIfArtificial)
{
    if (!D) {
        return;
    }
    switch (D.getTag()) {
    case DW_TAG_subroutine_type: {
        appendSubroutineNameAfter(D, Inner, SkipFirstParamIfArtificial, false, false);
        break;
    }
    case DW_TAG_array_type: {
        appendArrayType(D);
    ...
    case DW_TAG_LLVM_ptrauth_type: {
        ...
        PtrauthStream << "__ptrauth(" << getValOrNull(DW_AT_LLVM_ptrauth_key) << ", "
                      << getValOrNull(DW_AT_LLVM_ptrauth_address_discriminated) << ", 0x0"
                      << utohexstr(getValOrNull(DW_AT_LLVM_ptrauth_extra_discriminator), true) << options << ")";
...
static bool scopedTAGs(dwarf::Tag Tag)
{
    ...
    case dwarf::DW_TAG_typedef: // <--- FIXME: dwarf2cpp added typedef here for typedefs like std::string
        return true;
    default:
        break;
    }
...
bool DWARFTypePrinter::appendTemplateParameters(DWARFDie D, bool *FirstParameter)
{
    ...
    for (const DWARFDie &C : D) {
        ...
        if (C.getTag() == dwarf::DW_TAG_template_value_parameter) {
            ...
            if (T.getTag() == DW_TAG_pointer_type || T.getTag() == DW_TAG_reference_type) {
                continue;
            }
            ...
            else if (Name == "char" || (IsQualifiedChar = (Name == "unsigned char" || Name == "signed char"))) {
        ...
        if (C.getTag() == dwarf::DW_TAG_GNU_template_template_param) {
            const char *RawName = dwarf::toString(C.find(DW_AT_GNU_template_name), nullptr);
            assert(RawName);
            StringRef Name = RawName;
            Sep();
            OS << Name;
        ...
        Sep();
        appendQualifiedName(TypeAttr ? resolveReferencedType(C, *TypeAttr) : DWARFDie());
    }
...
void DWARFTypePrinter::decomposeConstVolatile(DWARFDie &N, DWARFDie &T, DWARFDie &C, DWARFDie &V)
{
...
void DWARFTypePrinter::appendConstVolatileQualifierAfter(DWARFDie N)
{
    ...
    if (T && T.getTag() == DW_TAG_subroutine_type) {
        appendSubroutineNameAfter(T, resolveReferencedType(T), false, C.isValid(), V.isValid());
    }
    else {
...
void DWARFTypePrinter::appendConstVolatileQualifierBefore(DWARFDie N)
{
    ...
    bool Leading =
        (!A || (A.getTag() != DW_TAG_pointer_type && A.getTag() != llvm::dwarf::DW_TAG_ptr_to_member_type)) &&
        !Subroutine;
    if (Leading) {
        if (C) {
            OS << "const ";
...
void DWARFTypePrinter::appendUnqualifiedName(DWARFDie D, std::string *OriginalFullName)
{
...
void DWARFTypePrinter::appendSubroutineNameAfter(DWARFDie D, DWARFDie Inner, bool SkipFirstParamIfArtificial,
                                                 bool Const, bool Volatile)
{
    ...
    bool RealFirst = true;
    for (DWARFDie P : D) {
        if (P.getTag() != DW_TAG_formal_parameter && P.getTag() != DW_TAG_unspecified_parameters) {
            return;
        }
